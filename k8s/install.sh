...
check_prerequisites() {
    ...
    echo "   • Production: maddigsys/falco-ai-alerts:v1.5.7"
}
...
show_secret_creation_instructions() {
    ...
    echo ""
    print_info "🤖 Default AI Model: tinyllama (ultra-fast, CPU-optimized)"
    echo "   • Size: ~637MB download, 6-8GB RAM required minimum"
    echo "   • Performance: 3-10s inference on CPU"
    echo "   • Use case: Reliable default for all environments"
    echo ""
    print_warning "☁️ Cloud Deployment: Ensure minimum 8GB RAM nodes!"
    echo "   • See k8s/CLOUD_DEPLOYMENT_GUIDE.md for platform-specific requirements"
    echo "   • AWS EKS: Use t3.xlarge+ instances"
    echo "   • GKE: Use n1-standard-4+ machines"  
    echo "   • AKS: Use Standard_D4s_v3+ VMs"
    echo ""
